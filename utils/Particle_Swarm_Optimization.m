...
function [ optimised_parameters ] = Particle_Swarm_Optimization (Bird_in_swarm, Number_of_quality_in_Bird, MinMaxRange, objfunc, availability_type, velocity_clamping_factor, cognitive_constant, social_constant, Min_Inertia_weight, Max_Inertia_weight, max_iteration)
...
for itr=1:max_iteration
    ...
    for p=1:Bird_in_swarm
        ...
        %return;
        availability(p,itr)=feval(objfunc,parameter);
        %availability(p,itr)=objfunc(parameter);
        if availability(p,itr)>0
          disp("****************************************");
